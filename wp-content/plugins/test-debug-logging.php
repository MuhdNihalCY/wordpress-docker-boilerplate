...
<body>
    ...
    if (function_exists('write_error_log')) {
        write_error_log('Test error message', [
            'error_code' => 999,
    ...
    if (function_exists('get_debug_logs')) {
        $logs = get_debug_logs(20);
        if ($logs && $logs !== 'No debug logs found.') {
        ...
        echo '<div class="error">✗ get_debug_logs() function not found</div>';
    }
    echo '</div>';
    ?>
    ...
    <div class="test-section">
        ...
        <button onclick="window.open('<?php echo admin_url('tools.php?page=debug-logs'); ?>', '_blank')">View Debug Logs in Admin</button>
        <button onclick="if(confirm('Clear all debug logs?')) { window.location.href = '?clear_logs=1'; }">Clear Debug Logs</button>
    </div>
    
    <?php
    // Handle clear logs request
    if (isset($_GET['clear_logs']) && function_exists('clear_debug_logs')) {
        clear_debug_logs();
...
write_error_log('Something went wrong', ['error_code' => 500]);
...
write_log('Debug message', 'debug');
    ...
    <div class="test-section">
        ...
        <pre>
WordPress Debug Log: <?php echo WP_CONTENT_DIR; ?>/debug.log
Custom Debug Log: <?php echo WP_CONTENT_DIR; ?>/debug-logs/custom-debug.log
        </pre>
    </div>
</body>
</html>
